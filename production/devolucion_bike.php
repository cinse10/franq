<?php 
	...
	$sql = "SELECT * FROM ((`tickets` INNER JOIN socios ON tickets.id_cliente = socios.id_socio) INNER JOIN empleados ON tickets.id_empledo=empleados.id_empleado) WHERE id_ticket=".$id_ticket;
  ...
  <div class="">
    <div class="page-title">
      <div class="title_left">
        <h3>Devolución de Productos Bikenet</h3>
      </div>
    </div>
                    ...
                    <tbody>
                      <?php 
                        ...
                        if (isset($_GET['id_ticket'])) {
							$id_ticket=$_GET['id_ticket'];
							$kueri = "SELECT * FROM `transacciones` INNER JOIN productos_bike ON transacciones.id_producto=productos_bike.id_producto WHERE id_ticket=".$id_ticket;
							...
							if($renglones!=0){
								...
								while($producto = mysqli_fetch_array($respuesta2)){
									echo "<tr>";
										echo "<th>".$producto['nombre_producto']."</th>";
...
function eliminar(id_transaccion) {
   ...
   function manda_eliminar(id_transaccion){
    ...
    $.ajax({
      ...
      data: {'id_ticket': id_ticket, 'id_transaccion': id_transaccion, 'marca' : 16},  
