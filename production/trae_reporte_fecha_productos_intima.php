<?php 
	...
	if (isset($_POST['fecha_inicio'])&&isset($_POST['fecha_fin'])) {
		...
		if ($_POST['id_empleado']==0) {
			$sql = "select * from tickets_intima INNER JOIN empleados ON tickets_intima.id_empledo=empleados.id_empleado INNER JOIN socios ON tickets_intima.id_cliente=socios.id_socio INNER JOIN transacciones_intima ON tickets_intima.id_ticket=transacciones_intima.id_ticket INNER JOIN productos ON transacciones_intima.id_producto = productos.id_producto where  fecha_ticket between ''".$inicio." 00:00:00' AND '".$fin." 23:59:59'";	
		}else{
			$sql = "select * from tickets_intima INNER JOIN empleados ON tickets_intima.id_empledo=empleados.id_empleado INNER JOIN socios ON tickets_intima.id_cliente=socios.id_socio INNER JOIN transacciones_intima ON tickets_intima.id_ticket=transacciones_intima.id_ticket INNER JOIN productos ON transacciones_intima.id_producto = productos.id_producto where  fecha_ticket between ''".$inicio." 00:00:00' AND '".$fin." 23:59:59'  AND empleados.id_empleado=".$_POST['id_empleado'];	
		}
		...
		if (isset($_POST['ticket'])) {
				...
				$direccion="";
				$leyenda_inferior="";
				$confi = "select * from configuracion where id_configuracion = 3";
