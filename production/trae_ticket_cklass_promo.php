<?php 
	...
	$direccion="";
	$leyenda_inferior="";
	$sql = "select * from configuracion where id_configuracion = '7'";
	...
	if (isset($_POST['id_ticket'])) {
		...
		$sql = "SELECT * FROM ((`tickets_cklass_promo` INNER JOIN socios_cklass ON tickets_cklass_promo.id_cliente = socios_cklass.id_socio) INNER JOIN empleados ON tickets_cklass_promo.id_empledo=empleados.id_empleado) WHERE id_ticket=".$id_ticket;
		...
		while($ticket = mysqli_fetch_array($respuesta)){
			...
			if($renglones!=0){
				...
				while($producto = mysqli_fetch_array($respuesta2)){
					if ($ban == 0) {
				      $chiquito = $producto['precio_venta'];
				      $ban = 1;
				    }else{
				      if ($chiquito < $producto['precio_venta']) {
				        $chiquito = $chiquito;
				      }else {
				        $chiquito = $producto['precio_venta'];
				      }
				...
				if ($ban==3) {
					$sindesc = $sumin-$chiquito;
					echo "<tr><td class='total'></td><td class='total'></td></tr>";
					echo "<tr><td class='total'>Total: </td><td class='precio centrado total'>$ ".$sumin."</td></tr>";	
					echo "<tr><td class='total'>Desc: </td><td class='precio centrado total'>$ ".$chiquito."</td></tr>";
					echo "<tr><td class='total'>Total con Desc: </td><td class='precio centrado total'>$ ".$sindesc."</td></tr>";
					echo "<tr><td class='total'>Saldo: </td><td class='precio centrado total'>$ ".$saldo."</td></tr>";
					$pagado = $sindesc - $saldo;
