  <?php 
	...
	if(isset($_POST['array'])){
	        ...
	        while ($productillo=mysqli_fetch_array($resp_producto)) {
	        	...
	        	if($stock>0){
	        		if($id_cliente==1){
	        			...
	        			$precio_producto = round($productillo['precio_compra']+($productillo['precio_compra']*0.16));
	        			...
	        			$precio_producto = ceil($productillo['precio_compra']*1.16 *1.20);
	        		}
					else{
	        			$precio_producto = $productillo['precio_publico_desc'];
	        		}
		...
		$sql_ticket = "INSERT INTO `apartado_bike`(`total_ticket`, `nombre_cliente`, `telefono_cliente`, `id_empledo`, `fecha_ticket`, `id_marca`, `total_abono`) VALUES ('".$total_pago."','".$nombre."','".$tel."',".$id_empleado.",'".$fecha_ticket."','2','".$abono."')";
		mysqli_query($conexion,$sql_ticket);
		$id_ticket = mysqli_insert_id($conexion);
			$kuerito = "INSERT INTO `abonos_bike`(`id_ticket`, `abono`, `fecha_abono` , `forma_pago`) VALUES (".$id_ticket.",'".$abono."', '".$fecha_ticket."','".$forma_pago."')";
			if(mysqli_query($conexion,$kuerito)){
		foreach ($productos as $posicion => $producto) {
				$bandera = 1;
				$kuerin = "INSERT INTO `transac_apartado_bike`(`id_ticket`, `id_producto`, `precio_venta`) VALUES (".$id_ticket.",".$producto.",'".$precios[$posicion]."')";
