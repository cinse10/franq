<?php 
	...
	if (isset($_POST['id_ticket'])) {
		...
		$sql = "SELECT * FROM ((`tickets_cubrebocas` INNER JOIN socios_vicky ON tickets_cubrebocas.id_cliente = socios_vicky.id_socio) INNER JOIN empleados ON tickets_cubrebocas.id_empledo=empleados.id_empleado) WHERE id_ticket=".$id_ticket;
		...
		while($ticket = mysqli_fetch_array($respuesta)){
			...
			echo "<tbody>";
			$kueri = "SELECT * FROM `transacciones_cubrebocas` INNER JOIN cubrebocas ON transacciones_cubrebocas.id_producto=cubrebocas.id_producto WHERE id_ticket=".$id_ticket;
			...
			if($renglones!=0){
				...
				while($producto = mysqli_fetch_array($respuesta2)){
					echo "<tr>";
						echo "<td>".$producto['descripcion']." - ".$producto['color']."</td>";
