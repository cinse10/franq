<?php 
  ...
  $sql = "SELECT * FROM ((`tickets_cklass_op` INNER JOIN socios_cklass ON tickets_cklass_op.id_cliente = socios_cklass.id_socio) INNER JOIN empleados ON tickets_cklass_op.id_empledo=empleados.id_empleado) WHERE id_ticket=".$id_ticket;
...
function eliminar(id_transaccion) {
  ...
  var id_ticket = document.getElementById("id_ticket").value;
    ...
    $.ajax({
      ...
      data: {'id_ticket': id_ticket, 'id_transaccion': id_transaccion, 'marca' : 3},  
