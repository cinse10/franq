 <?php 
  ...
  while($productin = mysqli_fetch_array($respuesta)) {
    ...
    $kuerin = "update productos set inventario_tmp_producto=".$inventario_producto." where id_producto=".$id_producto;
 ...
 ?> 
                        ...
                        <div class="form-group" >
                          ...
                          <div class="col-md-3 col-sm-3 col-xs-12" onclick="Registrar();" >
                            ...
                            <select class="selectpicker" data-show-subtext="true" data-live-search="true" id="codigo_barras" name="codigo_barras">
                              ...
                              <?php 
                                  include("conexion.php");
                                  $productos = mysqli_query($conexion,"SELECT * FROM `productos` WHERE inventario_producto!=0");
                                  while ($producto=mysqli_fetch_array($productos)) {
                                      $id_producto=$producto["id_producto"];
                                      $nombre_producto=$producto["nombre_producto"];
                                      $codigo_barras_producto=$producto["codigo_barras_producto"];

                                      echo '<option value="'.$id_producto.'" id="'.$id_producto.'">'.$nombre_producto.' /  '.$codigo_barras_producto.'</option>';
                                  }
                              ?>
                          </select>
                          </div>
        ...
        <div class="modal fade" id="modalNuevo" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" data-backdrop="static" data-keyboard="false">
            ...
            <div class="modal-content">
              ...
              <div class="modal-body">
                <select class="selectpicker" data-show-subtext="true" data-live-search="true" name="socio" id="socio" >
                  ...
                  <?php 
                    include("conexion.php");
                    $sql = "Select * from socios WHERE id_socio!=408 AND id_socio!=410 AND id_socio!=411 AND id_socio!=495";
...
<script type="text/javascript">
  ...
  function muestra_tdd(total,pagado){
    alertify.prompt("Ventana de cobro!","Total a pagar en efectivo: $ "+total+"\nIngresa pago recibido: ", "",
            function(evt, value ){
              Genera_ticketTDD(pagado);
            },
            function(){
  ...
  function traeSaldo(){
    $.ajax({
        ...
        data: {'id_cliente': cliente, 'marca': 3},  
  ...
  var nombre_cliente="# 164 CLIENTE PREFERENCIAL";
  var nombre_empleado="";
  var cliente=164;
  ...
  function creaTicket(id_ticket){
    var tfinal = Math.round(suma - saldo);
...
//ACTUALIZA PRODUCTO ML AL HACER UNA VENTA
function actualizaMl(id_producto){
    objeto = $('#tablita tbody').find('tr').each(function(i, el){ 
      AgregaArreglo(parseInt($(this).find('td').eq(0).text()));
    ...
    $.ajax({
      url: "./conector_ml/actualiza_intimaVentaMl.php",
      type: "POST",
      data:{id_producto: objeto[i].firstElementChild.textContent},       
    
      success: function(data){        
        console.log("el ID del producto es: "+data);
  ...
  //Actualiza Producto Linio al Hacer venta
  function actualizaLinio(id_producto){
    objeto = $('#tablita tbody').find('tr').each(function(i, el){ 
      AgregaArreglo(parseInt($(this).find('td').eq(0).text()));
    ...
    $.ajax({
      url: "./conector_linio/actualiza_intima_venta.php",
      type: "POST",
      data:{id_producto: objeto[i].firstElementChild.textContent},       
...
<script>
//directo si agregar
let slect = document.querySelector('.dropdown-menu');
slect.addEvenListener('click',Registrar());
</script>