<?php 
	...
	if (isset($_POST['id_ticket'])) {
		...
		while($ticket = mysqli_fetch_array($respuesta)){
			...
			echo "<tbody>";
			$kueri = "SELECT * FROM `transac_apartado_cklass` INNER JOIN productos_cklass ON transac_apartado_cklass.id_producto=productos_cklass.id_producto WHERE id_ticket=".$id_ticket;
			...
			if($renglones!=0){
				...
				while($producto = mysqli_fetch_array($respuesta2)){
					echo "<tr>";
						echo "<td>".$producto['modelo_producto']."-" .$producto['color_producto']."-".$producto['talla_producto']."</td>";
