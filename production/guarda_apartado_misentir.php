 <?php 
	...
	if(isset($_POST['array'])){
		...
		foreach ($productos as $posicion => $producto) {
			$sql_productos = "Select * from productos_misentir where id_producto=".$producto;
	        ...
	        while ($productillo=mysqli_fetch_array($resp_producto)) {
	        	...
	        	if($stock>0){
		        	...
		        	$stock = $stock -1 ;
		        	$update_producto = "UPDATE productos_misentir SET inventario_producto=".$stock." WHERE id_producto=".$productillo['id_producto'];
		        	mysqli_query($conexion,$update_producto);
	        	}else{
	        	}
	      	}
		}
		$sql_ticket = "INSERT INTO `apartado_misentir`(`total_ticket`, `nombre_cliente`, `telefono_cliente`, `id_empledo`, `fecha_ticket`, `id_marca`, `total_abono`) VALUES ('".$total_pago."','".$nombre."','".$tel."',".$id_empleado.",'".$fecha_ticket."','2','".$abono."')";
		...
		foreach ($productos as $posicion => $producto) {
				$bandera = 1;
				$kuerin = "INSERT INTO `transac_apartado_misentir`(`id_ticket`, `id_producto`, `precio_venta`) VALUES (".$id_ticket.",".$producto.",'".$precios[$posicion]."')";
