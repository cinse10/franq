<?php 
	...
	if ($_POST['codigo_barras']=="") {
		...
		$sql2 = "Select * from compras_desk_vicky where id_socio = '".$_POST['id_cliente']."'";
		...
		$sql = "Select * from productos_of_vicky where codigo_barras_producto='".$codigo."' or modelo_vf='".$codigo."'";
			...
			while($row = mysqli_fetch_array($resp)) {
				if ($row['inventario_producto']>0) {
					if ($row['inventario_tmp_producto']>0) {		
						...
						$kueri = "update productos_of_vicky set inventario_tmp_producto=".$tmp_inventario." where id_producto=".$row['id_producto'];
