<?php 
	...
	if (isset($_POST['fecha_inicio'])&&isset($_POST['fecha_fin'])) {
		...
		if ($_POST['id_empleado']==0) {
			$sql = "select * from tickets_cubrebocas INNER JOIN empleados ON tickets_cubrebocas.id_empledo=empleados.id_empleado INNER JOIN socios_vicky ON tickets_cubrebocas.id_cliente=socios_vicky.id_socio INNER JOIN transacciones_cubrebocas ON tickets_cubrebocas.id_ticket=transacciones_cubrebocas.id_ticket INNER JOIN cubrebocas ON transacciones_cubrebocas.id_producto = cubrebocas.id_producto where fecha_ticket between'".$inicio." 00:00:00' AND '".$fin." 23:59:59'";	
		}else{
			$sql = "select * from tickets_cubrebocas INNER JOIN empleados ON tickets_cubrebocas.id_empledo=empleados.id_empleado INNER JOIN socios_vicky ON tickets_cubrebocas.id_cliente=socios_vicky.id_socio INNER JOIN transacciones_cubrebocas ON tickets_cubrebocas.id_ticket=transacciones_cubrebocas.id_ticket INNER JOIN cubrebocas ON transacciones_cubrebocas.id_producto = cubrebocas.id_producto where fecha_ticket between'".$inicio." 00:00:00' AND '".$fin." 23:59:59' AND empleados.id_empleado=".$_POST['id_empleado'];	
		}
		...
		if ($num==0) {
			...
			while ($reporte = mysqli_fetch_array($resp)) {
				...
				$nombre_producto = $reporte['descripcion'].' - '.$reporte['talla'].' - '.$reporte['color'];
