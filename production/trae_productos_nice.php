<?php 
	...
	if ($_POST['codigo_barras']=="") {
		...
		$sql = "Select * from productos_nice where id_producto='".$codigo."'";
			...
			while($row = mysqli_fetch_array($resp)) {
				if ($row['inventario_producto']>0) {
					if ($row['inventario_tmp_producto']>0) {
						if ($_POST['id_cliente']!=4) {
							$precio_desc = $row['precio_producto']-($row['precio_producto']*0.10);
							echo "<tr>
									<td>".$row['id_producto']."</td>
									<td>".$row['nombre_producto'].'-'.$row['modelo_producto']."</td>
									<td class='precio'>$ ".$row['precio_producto']."</td>
									<td class='precio'>$ ".$precio_desc."</td>
									<td><span class='glyphicon glyphicon-trash borrar'/></td>
								 </tr>";
						}else{
							$precio_desc = $row['precio_producto']-($row['precio_producto']*0.35);
							echo "<tr>
									<td>".$row['id_producto']."</td>
									<td>".$row['nombre_producto'].'-'.$row['modelo_producto']."</td>
									<td class='precio'>$ ".$row['precio_producto']."</td>
