          ...
          <div class="">
            <div class="page-title">
              <div class="title_left">
                <h3>Consulta productos Berlei</h3>
              </div>
            </div>
                            ...
                            <table id="tablin" class="table table-striped table-bordered bulk_action">
                              ...
                              <tbody>
                                <?php 
                                  ...
                                  $sql = "select * from productos_berlei ";
                                  $requi = mysqli_query($conexion,$sql);
                                  while ($reg=mysqli_fetch_array($requi)) {
                                    echo "<tr>";
                                      echo "<th>".$reg['id_producto']."</th>";
                                      echo "<th>".$reg['modelo']."</th>";
                                      echo "<th>".$reg['descripcion']."</th>";
                                      echo "<th>$ ".$reg['precio_catalogo']."</th>";
                                      echo "<th>$ ".$reg['precio_afiliado']."</th>";
                                      echo "<th> ".$reg['talla']."</th>";
                                      echo "<th> ".$reg['color']."</th>";
                                      echo "<th>".$reg['inventario_producto']."</th>";
                                      echo "<th>".$reg['codigo_barras_producto']."</th>";
                                      

                                      
                                      if($reportes==2 || $reportes ==1){                                     
                                        echo "<th><a href='acciones_berlei.php?action=edita&id_producto=".$reg['id_producto']."'><i class='glyphicon glyphicon-pencil'></i></a>&nbsp&nbsp&nbsp
                                        </th>";
                                      }else{
                                        echo "<th><a href='plus_berlei.php?action=edita&id_producto=".$reg['id_producto']."'><i class='glyphicon glyphicon-pencil'></i></a>&nbsp&nbsp&nbsp
                                        </th>";
                                      }
                                    echo "</tr>";
                                  }
...
<script type="text/javascript">
  ...
  function exportTableToExcel(tabla, filename = ''){
      ...
      // Specify file name
      filename = filename?filename+'.xls':'ProductosBerlei_'+fechita+'.xls';
...
function HazCotizacion(id_cotizacion) {
        ...
        var salto_linea = 70;
            ...
            function(dispose) {
                    ...
                    var nombre_cotizacion= "ProductosBerlei_"+fechita+".pdf"
