          ...
          <div class="">
            <div class="page-title">
              <div class="title_left">
                <h3>Consulta de tickets Bike Store</h3>
              </div>
            </div>
                          ...
                          <table id="tabla_tickets" class="table table-striped table-bordered bulk_action">
                            ...
                            <tbody>
                              <?php 
                                ...
                                $sql = "SELECT * FROM ((`tickets` INNER JOIN socios ON tickets.id_cliente = socios.id_socio) INNER JOIN empleados ON tickets.id_empledo=empleados.id_empleado) ORDER BY fecha_ticket DESC";
                                ...
                                while ($reg=mysqli_fetch_array($requi)) {
                                  echo "<tr>";
                                    ...
                                    echo "<a  href='devolucion_bike.php?id_ticket=".$reg["id_ticket"]."' class='btn btn-default btn-sm'><i class='fa fa-cut'></i></a>&nbsp";
                                    $fechita = Date("Y-m-d");
                                    $separar = (explode(" ",$reg['fecha_ticket']));
                                    $fecha = $separar[0];
                                    $hora = $separar[1];
                                    if ($iddelempleadito ==1 || $iddelempleadito == 6 && $reg['total_ticket'] !=0 ) {
   ...
   function manda_eliminar(id_ticket){
    ...
    $.ajax({
      ...
      success: function(data){ 
        ...
        window.location="reimpresion_ticket_bike.php";
