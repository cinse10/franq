          ...
          <div class="">
            <div class="page-title">
              <div class="title_left">
                <h3>Consulta Devoluciones Chiqui Mundo</h3>
              </div>
            </div>
                            ...
                            <tbody>
                              <?php 
                                ...
                                if ($iddelempleadito !=0 ) {
                                  ...
                                  $sql = "SELECT * FROM (`devoluciones_chiqui_mundo` INNER JOIN socios_chiqui_mundo ON devoluciones_chiqui_mundo.id_cliente = socios_chiqui_mundo.id_socio) INNER JOIN transacciones_chiqui_mundo_devoluciones on devoluciones_chiqui_mundo.id_ticket = transacciones_chiqui_mundo_devoluciones.id_ticket INNER JOIN productos_chiqui_mundo on transacciones_chiqui_mundo_devoluciones.id_producto = productos_chiqui_mundo.id_producto
                                  ";
                                  //$sql = "SELECT * FROM (`devoluciones_terra` INNER JOIN socios_terra ON tickets_terra.id_cliente = socios_terra.id_socio) INNER JOIN transacciones_terra on tickets_terra.id_ticket = transacciones_terra.id_ticket INNER JOIN productos_terra on transacciones_terra.id_producto = productos_terra.id_producto where saldo !=0";
                                  ...
                                  $sql = "select * from productos_chiqui_mundo";
                                  $requi = mysqli_query($conexion,$sql);
                                  while ($reg=mysqli_fetch_array($requi)) {
                                    echo "<tr>";
                                      ...
                                      if($reportes==2){                                     
                                        echo "<th><a href='acciones_chiqui_mundo.php?action=edita&id_producto=".$reg['id_producto']."'><i class='glyphicon glyphicon-pencil'></i></a>&nbsp&nbsp&nbsp
                                        </th>";
                                      }else{
                                        echo "<th></th>";
                                      }
...
<script type="text/javascript">
  ...
  function exportTableToExcel(tabla, filename = ''){
      ...
      // Specify file name
      filename = filename?filename+'.xls':'Devoluciones Cklass_'+fechita+'.xls';
...
function HazCotizacion(id_cotizacion) {
        ...
        var salto_linea = 70;
            ...
            doc.text("Productos a Devolver",X+307,Y-20);
            ...
            function(dispose) {
                    ...
                    var nombre_cotizacion= "Devoluciones Cklass_"+fechita+".pdf"
