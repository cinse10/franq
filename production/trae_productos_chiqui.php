<?php 
	...
	if ($_POST['codigo_barras']=="") {
		...
		$sql = "Select * from productos_chiqui where id_producto='".$codigo."'";
					...
					if ($row['inventario_tmp_producto']>0) {		
						if ($_POST['id_cliente']==1 ) {
							echo "<tr>
									...
									<td>".$row['nombre_producto'].'-'.$row['codigo_barras'].'-'.$row['descripcion']."</td>								
									<td class='precio'>$ ".$row['precio_publico']."</td>
									<td class='precio'>$ ".$row['precio_publico']."</td>									
									<td><span class='glyphicon glyphicon-trash borrar'/></td>
								 </tr>";
							
						}elseif($_POST['id_cliente']==18){ 
							echo "<tr>
									<td>".$row['id_producto']."</td>
									<td>".$row['nombre_producto'].'-'.$row['codigo_producto'].'-'.$row['color_producto']."</td>
									<td class='precio'>$ ".$row['precio_publico']."</td>
									<td class='precio'>$ ".$row['precio_afiliado']."</td>
							
									<td><span class='glyphicon glyphicon-trash borrar'/></td>
								 </tr>";
						}elseif($_POST['id_cliente']==21){
							echo "<tr>
							<td>".$row['id_producto']."</td>
							<td>".$row['nombre_producto'].'-'.$row['codigo_producto'].'-'.$row['color_producto']."</td>
							<td class='precio'>$ ".$row['precio_afiliado']."</td>
							<td class='precio'>$ ".$row['precio_empleado']."</td>
						...
						$kueri = "update productos_chiqui set inventario_tmp_producto=".$tmp_inventario." where id_producto=".$row['id_producto'];
