<?php 
	...
	if(isset($_POST['array'])){
		...
		foreach ($productos as $posicion => $producto) {
			$sql_productos = "Select * from productos_betterware where id_producto=".$producto;
	        ...
	        while ($productillo=mysqli_fetch_array($resp_producto)) {
	        	...
	        	if($stock>0){
		        	...
		        	$stock = $stock -1 ;
		        	$update_producto = "UPDATE productos_betterware SET inventario_producto=".$stock." WHERE id_producto=".$productillo['id_producto'];
		...
		$sql_ticket = "INSERT INTO `tickets_betterware`(`total_ticket`, `id_cliente`, `forma_pago`, `id_empledo`, `fecha_ticket`, `id_marca`) VALUES ('".$total_pago."',".$id_cliente.",'".$forma_pago."',".$id_empleado.",'".$fecha_ticket."','9')";
		mysqli_query($conexion,$sql_ticket);
		$id_ticket = mysqli_insert_id($conexion);
		foreach ($productos as $posicion => $producto) {
			$kuerin = "INSERT INTO `transacciones_betterware`(`id_ticket`, `id_producto`, `precio_venta`) VALUES (".$id_ticket.",".$producto.",'".$precios[$posicion]."')";
