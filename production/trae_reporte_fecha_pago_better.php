<?php 
	...
	if (isset($_POST['fecha_inicio'])&&isset($_POST['fecha_fin'])) {
		...
		if ($_POST['id_empleado']==0) {
			$sql = "select * from tickets_betterware INNER JOIN empleados ON tickets_betterware.id_empledo=empleados.id_empleado INNER JOIN socios_better ON tickets_betterware.id_cliente=socios_better.id_socio where fecha_ticket between'".$inicio." 00:00:00' AND '".$fin." 23:59:59'";	
		}else{
			$sql = "select * from tickets_betterware INNER JOIN empleados ON tickets_betterware.id_empledo=empleados.id_empleado INNER JOIN socios_better ON tickets_betterware.id_cliente=socios_better.id_socio where fecha_ticket between'".$inicio." 00:00:00' AND '".$fin." 23:59:59' AND empleados.id_empleado=".$_POST['id_empleado'];	
		}
		...
		if ($num==0) {
			...
			if (!isset($_POST['ticket'])) {
				echo "<tr><th></th><th></th><th></th><th></th><th style='font-weight: bold; font-size:20px;'>Total</th><th style='font-weight: bold; font-size:20px;'>$ ".$suma."</th></tr>";
				echo '</tbody></table>';
			}
		}
		if (isset($_POST['ticket'])) {
				...
				$direccion="";
				$leyenda_inferior="";
				$confi = "select * from configuracion where id_configuracion = 10";
