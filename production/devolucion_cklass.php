<?php 
	...
	$sql = "SELECT * FROM ((`tickets_cklass` INNER JOIN socios_cklass ON tickets_cklass.id_cliente = socios_cklass.id_socio) INNER JOIN empleados ON tickets_cklass.id_empledo=empleados.id_empleado) WHERE id_ticket=".$id_ticket;
...
function eliminar(id_transaccion) {
   ...
   function manda_eliminar(id_transaccion){
    ...
    $.ajax({
      ...
      data: {'id_ticket': id_ticket, 'id_transaccion': id_transaccion, 'marca' : 1},  
