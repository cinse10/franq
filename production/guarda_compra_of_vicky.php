<?php 
	...
	if(isset($_POST['array'])){
		...
		$bandera = 0;
		$precios = array();
		$sql2 = "Select * from compras_desc_vicky where id_socio = '".$_POST['id_cliente']."'";
		...
		foreach ($productos as $posicion => $producto) {
			$sql_productos = "Select * from productos_of_vicky where id_producto=".$producto;
	        ...
	        while ($productillo=mysqli_fetch_array($resp_producto)) {
	        	...
	        	if($stock>0){
		        	...
		        	$stock = $stock -1 ;
		        	$update_producto = "UPDATE productos_of_vicky SET inventario_producto=".$stock." WHERE id_producto=".$productillo['id_producto'];
