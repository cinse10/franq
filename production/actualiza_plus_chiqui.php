<?php 
	...
	if ($_FILES['uno']['tmp_name']!=NULL | $_FILES['dos']['tmp_name']!=NULL | $_FILES['tres']['tmp_name']!=NULL) {
		...
		if ($_FILES['uno']['tmp_name']!=NULL ) {
			...
			if ($one != NULL) {
				unlink("chiquiMundo/".$nombre_uno);  
			}
			if (move_uploaded_file($guardado_uno, 'chiquiMundo/'.$nombre_uno)) {		
				$ban1 =1;
				$ban2=2;
				$ban3=3;
			}else{
		...
		if ($_FILES['dos']['tmp_name']!=NULL ) {
			...
			if ($two != NULL) {
				unlink("chiquiMundo/".$nombre_dos); 
			}
			if (move_uploaded_file($guardado_dos, 'chiquiMundo/'.$nombre_dos)) {		
				$ban1 =1;
				$ban2=2;
				$ban3=3;
			}else{
		...
		if ($_FILES['tres']['tmp_name']!=NULL ) {
			...
			if ($three != NULL) {
				unlink("chiquiMundo/".$nombre_tres);  
			}
			if (move_uploaded_file($guardado_tres, 'chiquiMundo/'.$nombre_tres)) {		
				$ban1 =1;
				$ban2=2;
				$ban3=3;
			}else{
