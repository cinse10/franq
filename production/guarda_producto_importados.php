<?php 
	...
	if (!is_numeric($precio_clave)) {
		...
		if(!validaRepetidos($codigo_barras_producto)){
			$sql = "INSERT INTO `productos_importados`(`modelo_producto`, `color_producto`, `talla_producto`, `precio_clave`, `precio_afiliado`, `precio_publico`, `codigo_barras_producto`, `catalogo_producto`, `marca`, `inventario_inicial_producto`, `inventario_producto`, `inventario_tmp_producto`) VALUES ('".$modelo_producto."', '".$color_producto."', '".$talla_producto."', '".$precio_clave."', '".$precio_afiliado."', '".$precio_publico."', '".$codigo_barras_producto."', '".$catalogo_producto."', '".$marca."', ".$inventario_producto.", '".$inventario_producto."', '".$inventario_producto."')";
				if(mysqli_query($conexion,$sql)){
					$mensaje = "El producto ".$modelo_producto." se ha registrado con éxito";
					echo '<script language="javascript">';
						echo 'alert("'.$mensaje.'");';
						echo 'window.location="agrega_productos_importados.php";';
					echo '</script>';
				}else{
					echo '<script language="javascript">';
	...
	function validaRepetidos($codigo_barras_producto){
		include("conexion.php");
		$sql = "select * from productos_importados where codigo_barras_producto='".$codigo_barras_producto."'";
