                          ...
                          <table id="tabla_tickets" class="table table-striped table-bordered bulk_action">
                            ...
                            <tbody>
                              <?php 
                                ...
                                $sql = "SELECT * FROM ((`tickets_terra_desc` INNER JOIN socios_terra ON tickets_terra_desc.id_cliente = socios_terra.id_socio) INNER JOIN empleados ON tickets_terra_desc.id_empledo=empleados.id_empleado) ORDER BY fecha_ticket DESC";
                                ...
                                while ($reg=mysqli_fetch_array($requi)) {
                                  echo "<tr>";
                                    ...
                                    echo "<th><button class='btn btn-default btn-sm' onclick='checaTicket(".$reg["id_ticket"].");'><i class='fa fa-print'></i></button>
                                          <a  href='devolucion_terra_desc.php?id_ticket=".$reg["id_ticket"]."' class='btn btn-default btn-sm'><i class='fa fa-cut'></i></a>&nbsp";
                                    $fechita = Date("Y-m-d");
  ...
  function checaTicket(id_ticket){
    ...
    $("#modalReimpresionTicket").modal('show');
     $.ajax({
      type: "POST",
      dataType: 'html',
      url: "trae_ticket_terra_desc.php",
  ...
  function eliminar(id_ticket) {
   ...
   function manda_eliminar(id_ticket){
    ...
    $.ajax({
      ...
      data: {'id_ticket': id_ticket, 'marca' : 11},  
      success: function(data){ 
        alert("Venta Eliminada con éxito");
        window.location="reimpresion_ticket_terra_desc.php";
