<?php 
	...
	if (!is_numeric($precio_producto)) {
		echo '<script language="javascript">';
			...
			echo 'window.location="agrega_productos_better.php";';
		echo '</script>';
	}else{

		if(!validaRepetidos($codigo_barras_producto)){
			$sql = "INSERT INTO `productos_betterware`(`nombre_producto`, `precio_catalogo`, `precio_compra`, `inventario_producto`, `inventario_tmp_producto`, `desc_producto`, `codigo_producto`) VALUES ('".$nombre_producto."', '".$precio_producto."', '".$precio_afi_producto."', '".$inventario_producto."', '".$inventario_producto."', '".$descripcion_producto."',  '".$codigo_barras_producto."')";
				if(mysqli_query($conexion,$sql)){
						$id_betterware = mysqli_insert_id($conexion);
						$kuerito = "INSERT INTO `base_betterware`(`id_betterware`, `inventario_better`) VALUES (".$id_betterware.", ".$inventario_producto.")";
						if(mysqli_query($conexion,$kuerito)){
							$mensaje = "El producto ".$nombre_producto." se ha registrado con éxito";
							echo '<script language="javascript">';
								echo 'alert("'.$mensaje.'");';
								echo 'window.location="consulta_productos_better.php";';
							echo '</script>';
						}else{ 
							echo '<script language="javascript">';
	...
	function validaRepetidos($codigo_barras_producto){
		include("conexion.php");
		$sql = "select * from productos_betterware where codigo_producto='".$codigo_barras_producto."'";
