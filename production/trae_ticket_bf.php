<?php 
	...
	$direccion="";
	$leyenda_inferior="";
	$sql = "select * from configuracion where id_configuracion = '3'";
	...
	if (isset($_POST['id_ticket'])) {
		...
		$sql = "SELECT * FROM ((`tickets_intima_bf` INNER JOIN socios ON tickets_intima_bf.id_cliente = socios.id_socio) INNER JOIN empleados ON tickets_intima_bf.id_empledo=empleados.id_empleado) WHERE id_ticket=".$id_ticket;
		...
		while($ticket = mysqli_fetch_array($respuesta)){
			...
			if($renglones!=0){
				...
				echo "<tr><td class='total'>Descuento: </td><td class='precio centrado total'>$ ".$ticket['desc_intima']."</td></tr>";
				echo "<tr><td class='total'>Total con Descuento: </td><td class='precio centrado total'>$ ".$ticket['total_ticket']."</td></tr>";
				echo "</tbody>";
				echo "</table>";
				echo "<br>";
				echo "--------------------------------------------------";
