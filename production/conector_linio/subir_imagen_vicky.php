        ...
        $id_producto = $_GET['id_producto'];
            ...
            while ($reg=mysqli_fetch_array($requi)) {
                ...
                $cod_linio=$reg['cod_linio'];
                    ...
                    $accion = 'Image';
                    $formato = 'JSON';
                    if ($uno!=NULL && $dos != NULL && $tres != NULL || $uno!='' && $dos != '' && $tres != '' ) {
                        $request_xml = '<?xml version="1.0" encoding="UTF-8" ?>
                                        <Request>
                                            <ProductImage>
                                                <SellerSku>'.$cod_linio.'</SellerSku>
                                                <Images>
                                                    <Image>http://13.59.1.71/franq/production/intima/vicky'.$uno.'</Image>
                                                    <Image>http://13.59.1.71/franq/production/intima/vicky'.$dos.'</Image>
                                                    <Image>http://13.59.1.71/franq/production/intima/vicky'.$tres.'</Image>
                                                </Images>
                                            </ProductImage>
                                        </Request>   
                        ...
                        ';
                        $prueba=1;
                    }elseif ($uno!=NULL && $dos !=NULL && $tres == NULL || $uno!='' && $dos !='' && $tres == '') {
                        $request_xml = '<?xml version="1.0" encoding="UTF-8" ?>
                                        <Request>
                                            <ProductImage>
                                                <SellerSku>'.$cod_linio.'</SellerSku>
