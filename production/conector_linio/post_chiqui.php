<?php
    include("../conexion.php");
        $id_producto = $_GET['id_producto'];
            ...
            while ($reg=mysqli_fetch_array($requi)) {
                ...
                $precio_producto = $reg['precio_publico'];
    ...
    switch ($tipo_producto) {
        //cunas
        case "19405":
            $precio_linio = ceil(($precio_producto + $costo_guia) * 1.20);
            break;    
            //mantas y cobijas    
        case "19409":
            $precio_linio = ceil(($precio_producto + $costo_guia) * 1.20);
   ...
   /* $sql1 = "select * from productos_chiqui WHERE cod_linio="."'$cod_linio'";
            $requis = mysqli_query($conexion,$sql1);
            while ($regss=mysqli_fetch_array($requis)) {
                $codigo_linio = $regss['cod_linio'];
                //print_r($codigo_linio);
    ...
    $request_xml = '<?xml version="1.0" encoding="UTF-8" ?>
                    <Request>
                      <Product>
                        ...
                        <Name>'.$nombre_producto.'-Chiqui Mundo</Name>
         ...
         $url = '';
           ...
           if ($pos === NULL) {
                echo '<script language="javascript">';
                    ...
                    echo 'window.location="../plus_chiqui.php?action=edita&id_producto='.$id_producto.'";';
                echo '</script>';
            }else{
                $band=1;
                $sql1 = "UPDATE `productos_chiqui` SET `sub_linio`='".$band."' WHERE id_producto=".$id_producto;
