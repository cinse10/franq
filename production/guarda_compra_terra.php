<?php 
	...
	if(isset($_POST['array'])){
	        		...
	        		if ($_POST['id_cliente']==1 || $_POST['id_cliente']==2) {
	        			...
	        			if($libro == 'IMPORTADOS'){
	        				if ($_POST['id_cliente']==30) {
	        					...
	        					$precio_producto = round($productillo['precio_clave']-($productillo['precio_clave']*0.24),1,PHP_ROUND_HALF_UP);
	        				}
							else{
		        			...
		        			if ($lista==1) {
				        		...
				        		elseif ($_POST['id_cliente']==284) {
				        			$precio_producto = round($row['precio_oferta']-($row['precio_oferta']*0.22),1,PHP_ROUND_HALF_UP);
				        		}
		...
		$sql_ticket = "INSERT INTO `tickets_terra`(`total_ticket`, `id_cliente`, `forma_pago`, `id_empledo`, `fecha_ticket`, `id_marca`, `saldito`) VALUES ('".$total_final."',".$id_cliente.",'".$forma_pago."',".$id_empleado.",'".$fecha_ticket."','5','".$saldo."')";
		mysqli_query($conexion,$sql_ticket);
		$id_ticket = mysqli_insert_id($conexion);
		foreach ($productos as $posicion => $producto) {
			$kuerin = "INSERT INTO `transacciones_terra`(`id_ticket`, `id_producto`, `precio_venta`) VALUES (".$id_ticket.",".$producto.",'".$precios[$posicion]."')";
			// echo $kuerin;
			// echo "<br><br>";
