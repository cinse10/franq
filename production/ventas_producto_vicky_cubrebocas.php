 <?php 
  ...
  $kuerito =  "Select * from productos_vicky";
  ...
  while($productin = mysqli_fetch_array($respuesta)) {
    ...
    $kuerin = "update productos_vicky set inventario_tmp_producto=".$inventario_producto." where id_producto=".$id_producto;
                        ...
                        <div class="form-group">
                          ...
                          <div class="col-md-3 col-sm-3 col-xs-12" onclick="Registrar();">
                            <select class="selectpicker" data-show-subtext="true" data-live-search="true" id="codigo_barras" name="codigo_barras">
                                ...
                                <?php 
                                    include("conexion.php");
                                    $productos = mysqli_query($conexion,"SELECT * FROM `productos_vicky` WHERE id_producto = 2 OR id_producto= 1617 OR id_producto = 1618 OR id_producto = 221 OR id_producto = 2389 OR id_producto = 2422 OR id_producto = 2291 AND  inventario_producto!=0 ");
                                    while ($producto=mysqli_fetch_array($productos)) {
                                        $id_producto=$producto["id_producto"];
                                        $modelo_vf=$producto["modelo_vf"];
                                        $talla_vf=$producto["descripcion_vf"];
                                        $color_vf=$producto["color_vf"];
                                        $codigo_barras_producto=$producto["codigo_barras_producto"];


                                        echo '<option value="'.$codigo_barras_producto.'" id="'.$id_producto.'">'.$modelo_vf.'-'.$talla_vf.'-'.$color_vf.' /  '.$codigo_barras_producto.'</option>';
                                    }
                                ?>
                            </select>
            ...
            <div class="modal-content">
              ...
              <div class="modal-body">
                <select class="selectpicker" data-show-subtext="true" data-live-search="true" name="socio" id="socio">
                  ...
                  <?php 
                    include("conexion.php");
                    $sql = "SELECT * FROM `socios_vicky` WHERE id_socio = 1";
...
<script type="text/javascript">
  ...
  function Cargar(){
    $('#datagrid').load('trae_productos_vicky.php');    
  }
  function Limpiar(){
    $("#codigo_barras").val("");
  }
  ...
  function traeSaldo(){
    $.ajax({
        ...
        data: {'id_cliente': cliente, 'marca': 5},  
