<?php 
	...
	$sql = "SELECT * FROM ((`tickets_vicky` INNER JOIN socios_vicky ON tickets_vicky.id_cliente = socios_vicky.id_socio) INNER JOIN empleados ON tickets_vicky.id_empledo=empleados.id_empleado) WHERE id_ticket=".$id_ticket;
...
function eliminar(id_transaccion) {
   ...
   function manda_eliminar(id_transaccion){
    ...
    $.ajax({
      ...
      data: {'id_ticket': id_ticket, 'id_transaccion': id_transaccion, 'marca' : 4},  
