                              ...
                              <table id="tabla_tickets" class="table table-striped table-bordered bulk_action">
                                ...
                                <tbody>
                                  <?php 
                                    ...
                                    $sql = "SELECT * FROM `apartado_bike` INNER JOIN empleados ON apartado_bike.id_empledo=empleados.id_empleado ORDER BY fecha_ticket DESC";
                                    ...
                                    while ($reg=mysqli_fetch_array($requi)) {
                                      ...
                                      if ($resta!=0) {
                                        ...
                                        echo "<th><button class='btn btn-default btn-sm' onclick='checaTicket(".$reg["id_ticket"].");'><i class='fa fa-print'></i></button> 
                                          <a  href='ver_abonos_bike.php?id_ticket=".$reg["id_ticket"]."' class='btn btn-default btn-sm'><i class='fa fa-paste'></i></a>&nbsp
                                          </th>";
                                          $sqld = "select * from empleados where id_empleado=".$iddelempleadito;
                                          $resultado = mysqli_query($conexion,$sqld);
      while($permiso = mysqli_fetch_array($resultado)){
        if($iddelempleadito!=2 && $iddelempleadito !=11){
          echo "<th><button class='btn btn-default btn-sm' onclick='eliminar(".$reg["id_ticket"].");'><i class='fa fa-trash'></i></button></th>";
        }else{
          echo "<th></th>";
                                  ...
                                  <?php 
                                    ...
                                    while ($reg=mysqli_fetch_array($requi)) {
                                      ...
                                      if ($resta==0) {
                                        ...
                                        echo "<th>$ ".$reg['total_ticket']."</th>";
                                        echo "<th><button class='btn btn-default btn-sm' onclick='checaTicket(".$reg["id_ticket"].");'><i class='fa fa-print'></i></button> 
                                          <a  href='ver_abonos_bike2.php?id_ticket=".$reg["id_ticket"]."' class='btn btn-default btn-sm'><i class='fa fa-paste'></i></a>&nbsp
...
<script type="text/javascript">
  ...
  function eliminar(id_ticket) {
     alertify.confirm()
     alertify.confirm('Eliminar', '¿Desea eliminar apartado?', 
      function(){ 
  ...
  function manda_eliminar(id_ticket){
    ...
    $.ajax({
      ...
      data: {'id_ticket': id_ticket, 'marca' : 2},  
      success: function(data){ 
        alert("Apartado Eliminado con éxito");
        window.location="reimpresion_apartado_bike.php";
