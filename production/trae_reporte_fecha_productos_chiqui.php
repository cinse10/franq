<?php 
	...
	if (isset($_POST['fecha_inicio'])&&isset($_POST['fecha_fin'])) {
		...
		if ($_POST['id_empleado']==0) {
			$sql = "select * from tickets_chiqui INNER JOIN empleados ON tickets_chiqui.id_empledo=empleados.id_empleado INNER JOIN socios_chiqui ON tickets_chiqui.id_cliente=socios_chiqui.id_socio INNER JOIN transacciones_chiqui ON tickets_chiqui.id_ticket=transacciones_chiqui.id_ticket INNER JOIN productos_chiqui ON transacciones_chiqui.id_producto = productos_chiqui.id_producto where fecha_ticket between'".$inicio." 00:00:00' AND '".$fin." 23:59:59'";	
		}else{
			$sql = "select * from tickets_chiqui INNER JOIN empleados ON tickets_chiqui.id_empledo=empleados.id_empleado INNER JOIN socios_chiqui ON tickets_chiqui.id_cliente=socios_chiqui.id_socio INNER JOIN transacciones_chiqui ON tickets_chiqui.id_ticket=transacciones_chiqui.id_ticket INNER JOIN productos_chiqui ON transacciones_chiqui.id_producto = productos_chiqui.id_producto where fecha_ticket between'".$inicio." 00:00:00' AND '".$fin." 23:59:59' AND empleados.id_empleado=".$_POST['id_empleado'];	
		}
		...
		if ($num==0) {
			...
			while ($reporte = mysqli_fetch_array($resp)) {
				...
				$nombre_producto = $reporte['nombre_producto'].' - '.$reporte['codigo_barras'].' - '.$reporte['descripcion'];
		...
		if (isset($_POST['ticket'])) {
				...
				$direccion="";
				$leyenda_inferior="";
				$confi = "select * from configuracion where id_configuracion = 11";
