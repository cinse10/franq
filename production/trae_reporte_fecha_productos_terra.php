<?php 
	...
	if (isset($_POST['fecha_inicio'])&&isset($_POST['fecha_fin'])) {
		...
		if ($_POST['id_empleado']==0) {
			$sql = "select * from tickets_terra INNER JOIN empleados ON tickets_terra.id_empledo=empleados.id_empleado INNER JOIN socios_terra ON tickets_terra.id_cliente=socios_terra.id_socio INNER JOIN transacciones_terra ON tickets_terra.id_ticket=transacciones_terra.id_ticket INNER JOIN productos_terra ON transacciones_terra.id_producto = productos_terra.id_producto where fecha_ticket between'".$inicio." 00:00:00' AND '".$fin." 23:59:59'";	
		}else{
			$sql = "select * from tickets_terra INNER JOIN empleados ON tickets_terra.id_empledo=empleados.id_empleado INNER JOIN socios_terra ON tickets_terra.id_cliente=socios_terra.id_socio INNER JOIN transacciones_terra ON tickets_terra.id_ticket=transacciones_terra.id_ticket INNER JOIN productos_terra ON transacciones_terra.id_producto = productos_terra.id_producto where fecha_ticket between'".$inicio." 00:00:00' AND '".$fin." 23:59:59' AND empleados.id_empleado=".$_POST['id_empleado'];	
		}
		...
		if ($num==0) {
			...
			while ($reporte = mysqli_fetch_array($resp)) {
				...
				$nombre_producto = $reporte['modelo_producto'].' - '.$reporte['talla_producto'].' - '.$reporte['color_producto'];
