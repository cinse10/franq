          ...
          <div class="">
            <div class="page-title">
              <div class="title_left">
                <h3>Consulta de tickets Ofertas</h3>
              </div>
            </div>
                          ...
                          <table id="tabla_tickets" class="table table-striped table-bordered bulk_action">
                            ...
                            <tbody>
                              <?php 
                                ...
                                $sql = "SELECT * FROM ((`tickets_desc_vicky` INNER JOIN socios_vicky ON tickets_desc_vicky.id_cliente = socios_vicky.id_socio) INNER JOIN empleados ON tickets_desc_vicky.id_empledo=empleados.id_empleado) ORDER BY fecha_ticket DESC";
                                ...
                                while ($reg=mysqli_fetch_array($requi)) {
                                  echo "<tr>";
                                    ...
                                    echo "<th><button class='btn btn-default btn-sm' onclick='checaTicket(".$reg["id_ticket"].");'><i class='fa fa-print'></i></button>
                                          <a  href='devolucion_desc_vicky.php?id_ticket=".$reg["id_ticket"]."' class='btn btn-default btn-sm'><i class='fa fa-cut'></i></a>&nbsp";
                                    $fechita = Date("Y-m-d");
                                    $separar = (explode(" ",$reg['fecha_ticket']));
                                    $fecha = $separar[0];
                                    $hora = $separar[1];
                                    if ($iddelempleadito ==1 && $reg['total_ticket'] !=0  && $fecha == $fechita) {
  ...
  function eliminar(id_ticket) {
   ...
   function manda_eliminar(id_ticket){
    ...
    $.ajax({
      ...
      data: {'id_ticket': id_ticket, 'marca' : 8},  
      success: function(data){ 
        alert("Venta Eliminada con éxito");
        window.location="reimpresion_desc_vicky.php";
