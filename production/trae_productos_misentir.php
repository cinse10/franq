<?php 
	...
	if ($_POST['codigo_barras']=="") {
		...
		$sql = "Select * from productos_misentir where id_producto='".$codigo."'";
			...
			while($row = mysqli_fetch_array($resp)) {
				if ($row['inventario_producto']>0) {
					if ($row['inventario_tmp_producto']>0) {		
						...
						$kueri = "update productos_misentir set inventario_tmp_producto=".$tmp_inventario." where id_producto=".$row['id_producto'];
