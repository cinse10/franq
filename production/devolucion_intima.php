<?php 
	...
	$sql = "SELECT * FROM ((`tickets_intima` INNER JOIN socios ON tickets_intima.id_cliente = socios.id_socio) INNER JOIN empleados ON tickets_intima.id_empledo=empleados.id_empleado) WHERE id_ticket=".$id_ticket;
...
function eliminar(id_transaccion) {
   ...
   function manda_eliminar(id_transaccion){
    ...
    $.ajax({
      ...
      data: {'id_ticket': id_ticket, 'id_transaccion': id_transaccion, 'marca' : 8},  
