<?php 
	...
	if (isset($_POST['fecha_inicio'])&&isset($_POST['fecha_fin'])) {
		...
		if ($_POST['id_empleado']==0) {
			$sql = "select * from tickets_desc_vicky INNER JOIN empleados ON tickets_desc_vicky.id_empledo=empleados.id_empleado INNER JOIN socios_vicky ON tickets_desc_vicky.id_cliente=socios_vicky.id_socio INNER JOIN transacciones_desc_vicky ON tickets_desc_vicky.id_ticket=transacciones_desc_vicky.id_ticket INNER JOIN productos_of_vicky ON transacciones_desc_vicky.id_producto = productos_of_vicky.id_producto where fecha_ticket between'".$inicio." 00:00:00' AND '".$fin." 23:59:59'";	
		}else{
			$sql = "select * from tickets_desc_vicky INNER JOIN empleados ON tickets_desc_vicky.id_empledo=empleados.id_empleado INNER JOIN socios_vicky ON tickets_desc_vicky.id_cliente=socios_vicky.id_socio INNER JOIN transacciones_desc_vicky ON tickets_vicky.id_ticket=transacciones_desc_vicky.id_ticket INNER JOIN productos_vicky ON transacciones_desc_vicky.id_producto = productos_desc_vicky.id_producto where fecha_ticket between'".$inicio." 00:00:00' AND '".$fin." 23:59:59' AND empleados.id_empleado=".$_POST['id_empleado'];	
		}
		...
		if ($num==0) {
			...
			while ($reporte = mysqli_fetch_array($resp)) {
				...
				$nombre_producto = $reporte['descripcion_vf'].' - '.$reporte['talla_vf'].' - '.$reporte['color_vf'];
