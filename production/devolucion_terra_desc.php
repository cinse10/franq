<?php 
	...
	$sql = "SELECT * FROM ((`tickets_terra_desc` INNER JOIN socios_terra ON tickets_terra_desc.id_cliente = socios_terra.id_socio) INNER JOIN empleados ON tickets_terra_desc.id_empledo=empleados.id_empleado) WHERE id_ticket=".$id_ticket;
...
function eliminar(id_transaccion) {
   ...
   function manda_eliminar(id_transaccion){
    ...
    $.ajax({
      ...
      data: {'id_ticket': id_ticket, 'id_transaccion': id_transaccion, 'marca' : 7},  
